...
const postforgotPassword = async (req, res) => {
  ...
  if (!User) {
    return res.status(404).render("forget-password", {
      message: "Email n'existe pas",
      });
  }
  const secret = jwtsecret + User.mot_de_passe;
  const payload = {
    email: User.email,
    id: User.id,
  ...
  // on va envoyer un email avec un lien pour réinitialiser le mot de passe
  // on va générer un token
};

async function sendMail(email , link, nom) {
